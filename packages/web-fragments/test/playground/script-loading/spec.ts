...
test('script loading in fragments', async ({ page }) => {
	...
	await step('ensure that child inline scripts executed correctly', async () => {
		...
		// TODO: we currently don't have ability to do this correctly
		// see the TODO in reframed.ts in node.prototype.appendChild
	...
	await step(`ensure all script loaded in fragment's JS context and not in the main context`, async () => {
		const frame = await page.frame({ name: 'script-loading' });
		...
		expect(await frame?.evaluate(() => window.SCRIPT_COUNTER)).toBe(9);
